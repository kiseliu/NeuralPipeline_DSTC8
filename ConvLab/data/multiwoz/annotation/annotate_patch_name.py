...
def add_name_annotation(data, database):
    ...
    for session_id, session in data.items():
        ...
        for i, turn in enumerate(session['log']):
            ...
            exist_name_idx = {x[2]: utt.lower().index(x[2].lower()) for x in span_info if
                              x[1] == 'Name' or x[1] == 'Dest' or x[1] == 'Depart'}
            name2idx = {}
                ...
                for name in name_set:
                    if phrase_in_utt(name, utt):
                        ...
                        if exist_name:
                            ...
                            word_overlap = [x for x in exist_name_idx.keys() if
                                            name.lower() in x.lower() or x.lower() in name.lower()]
...
if __name__ == '__main__':
    ...
    write_zipped_json('annotated_user_da_with_span_full_patchName.json.zip',
                      'annotated_user_da_with_span_full_patchName.json', data)
