    ...
    if len(slot_val_s) > 0:
        ...
        for annot_seg in annot_segs:
            ...
            if annot_seg.find('=') > 0:
                ...
                annot_slot = annot_seg[left_index:annot_seg.find('=', left_index)].strip(
                    ' ')  # annot_seg.split('=')[0].strip(' ')
...
def prepare_query_res_pairs(params):
    ...
    for key in raw_data.keys()[0:]:
        ...
        while turn_id < len(dialog['log']):
            ...
            f.write(
                key + "\t" + str(turn_id) + "\t" + txt + "\t" + ','.join(usr_acts) + "\t" + ','.join(sys_acts) + "\n")
            turn_id += 2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--raw_file_path', dest='raw_file_path', type=str,
                        default='./multiwoz/annotated_user_da_with_span_100sample.json', help='path to data file')
    parser.add_argument('--dia_act_slot', dest='dia_act_slot', type=str, default='./multiwoz/dialog_act_slot.txt',
                        help='path to data file')

    parser.add_argument('--txt_dia_act_file', dest='txt_dia_act_file', type=str,
                        default='./multiwoz/annotated_user_utts_18k.txt', help='path to data file')
