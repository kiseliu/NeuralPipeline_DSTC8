...
class EncoderGRUATTN(BaseRNN):
    ...
    def forward(self, residual_var, input_var, turn_feat, mask=None, init_state=None, input_lengths=None):
        ...
        if require_embed:
            ...
            input_cat = th.cat([input_var, residual_var, turn_feat],
                               2)  # (batch_size, max_dlg_len, dlg_cell_size+2*utt_cell_size)
        else:
        ...
        if require_rnn:
            ...
            logit = self.attn(h.contiguous().view(-1, 2 * self.nhid_attn)).view(h.size(0),
                                                                                h.size(1))  # (batch_size, max_dlg_len)
            # if mask is not None:
            ...
            logit = self.attn(embedded.contiguous().view(input_cat.size(0) * input_cat.size(1), -1)).view(
                input_cat.size(0), input_cat.size(1))
            if mask is not None:
                logit_mask = mask.view(input_cat.size(0), input_cat.size(1))
                logit_mask = -999.0 * logit_mask
                logit = logit_mask + logit
...
class SelectionClassifier(nn.Module):
    ...
    def forward(self, proj_outs):
        outs = [decoder.forward(proj_outs).unsqueeze(1) for decoder in
                self.sel_decoders]  # outcome_len*(batch_size, 1, outcome_vocab_size)
