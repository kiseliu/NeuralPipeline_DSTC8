...
class PTRUNK(nn.Module):
    ...
    def print_loss(self):
        ...
        self.print_every += 1
        return 'L:{:.2f}, VL:{:.2f},GL:{:.2f}, PL:{:.2f}'.format(print_loss_avg, print_loss_vac, print_loss_gate,
                                                                 print_loss_ptr)

    def save_model(self, dec_type):
        name_data = "KVR/" if self.task == '' else "BABI/"
        if USEKB:
            directory = 'save/PTR_KB-' + name_data + str(self.task) + 'HDD' + str(self.hidden_size) + 'DR' + str(
                self.dropout) + 'L' + str(self.n_layers) + 'lr' + str(self.lr) + str(dec_type)
        else:
            directory = 'save/PTR_noKB-' + name_data + str(self.task) + 'HDD' + str(self.hidden_size) + 'DR' + str(
                self.dropout) + 'L' + str(self.n_layers) + 'lr' + str(self.lr) + str(dec_type)
    ...
    def train_batch(self, input_batches, input_lengths, target_batches,
        ...
        if use_teacher_forcing:
            ...
            for t in range(max_target_length):
                ...
                next_in = [top_ptr_i.squeeze()[i].data[0] if (gate.squeeze()[i].data[0] >= 0.5) else topvi.squeeze()[i]
    ...
    def evaluate_batch(self, batch_size, input_batches, input_lengths, target_batches, target_lengths, target_index,
        ...
        for t in range(self.max_r):
            ...
            topp, toppi = decoder_ptr.data.topk(1)
            top_ptr_i = torch.gather(input_batches, 0, toppi.view(1, -1))
            next_in = [top_ptr_i.squeeze()[i].data[0] if (gate.squeeze()[i].data[0] >= 0.5) else topvi.squeeze()[i] for
    ...
    def evaluate(self, dev, avg_best, BLEU=False):
        ...
        for j, data_dev in pbar:
            words = self.evaluate_batch(len(data_dev[1]), data_dev[0], data_dev[1], data_dev[2], data_dev[3],
                                        data_dev[4], data_dev[5], data_dev[6])
            acc = 0
            w = 0
            temp_gen = []
...
class EncoderRNN(nn.Module):
    ...
    def get_state(self, input):
        ...
        h0_encoder = Variable(
            torch.zeros(self.n_layers, batch_size, self.hidden_size))  ### * self.num_directions = 2 if bi
        if USE_CUDA:
