...
class Mem2Seq(nn.Module):
    ...
    def save_model(self, dec_type):
        name_data = "KVR/" if args['dataset'] == 'kvr' else "WOZ/" if args['dataset'] == 'woz' else "BABI/"
        directory = self.root + '/save/mem2seq-' + name_data + str(self.task) + 'HDD' + str(
            self.hidden_size) + 'BSZ' + str(args['batch']) + 'DR' + str(self.dropout) + 'L' + str(
            self.n_layers) + 'lr' + str(self.lr) + str(dec_type)
        if not os.path.exists(directory):
            os.makedirs(directory)
    ...
    def train_batch(self, input_batches, input_lengths, target_batches,
        ...
        if use_teacher_forcing:
            ...
            for t in range(max_target_length):
                ...
                if batch_size > 1:
                    next_in = [top_ptr_i.squeeze()[i].item() if (toppi.squeeze()[i].item() < input_lengths[i] - 1) else
                               topvi.squeeze()[i].item() for i in range(batch_size)]
                else:
    ...
    def evaluate(self, dev, avg_best, BLEU=False):
        ...
        if args['dataset'] == 'kvr':
            ...
            if int(args["task"]) != 6:
                global_entity_list = entityList(self.root + '/data/dialog-bAbI-tasks/dialog-babi-kb-all.txt',
                                                int(args["task"]))
            else:
                global_entity_list = entityList(self.root + '/data/dialog-bAbI-tasks/dialog-babi-task6-dstc2-kb.txt',
                                                int(args["task"]))
