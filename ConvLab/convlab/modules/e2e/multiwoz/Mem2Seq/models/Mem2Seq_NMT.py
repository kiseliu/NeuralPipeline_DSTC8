...
class Mem2Seq(nn.Module):
    ...
    def save_model(self, dec_type):
        directory = 'save/mem2seq_' + 'HDD' + str(self.hidden_size) + 'BSZ' + str(self.batch_size) + 'DR' + str(
    ...
    def evaluate_batch(self, batch_size, input_batches, input_lengths, target_batches, target_lengths, target_index,
        ...
        for t in range(self.max_r):
            ...
            top_ptr_i = torch.gather(input_batches, 0, Variable(toppi.view(1, -1)))
            next_in = [
                top_ptr_i.squeeze()[i].data[0] if (toppi.squeeze()[i] < input_lengths[i] - 1) else topvi.squeeze()[i]
